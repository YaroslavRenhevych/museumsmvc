...
@WebAppConfiguration
public class GetCreatePicturePageStepDefs {
	@Autowired
	...
	public void before() {
	    ...
	    Assert.assertNotNull(wac.getBean("pictureController"));
	}
	
	@When("^the client calls \"(.*?)\" for picture creation$")
	...
	@Then("^the client receives status code (\\d+) from create picture page$")
	...
	@And("^the client forwarded to url \"([^\"]*)\" to create picture page$")
	...
	@And("the client gets view with name \"(.*?)\" for create picture page")
