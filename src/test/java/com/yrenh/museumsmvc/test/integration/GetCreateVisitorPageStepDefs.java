...
@WebAppConfiguration
public class GetCreateVisitorPageStepDefs {
	@Autowired
	...
	public void before() {
	    ...
	    Assert.assertNotNull(wac.getBean("visitorController"));
	}
	
	@When("^the client calls \"(.*?)\" for visitor creation$")
	...
	@Then("^the client receives status code of (\\d+) from create visitor page$")
	...
	@And("^the client forwarded to url \"([^\"]*)\" to create visitor page$")
	...
	@And("the client gets view with name \"(.*?)\" for create visitor page")
